...
async function main() {
  ...
  server.addEventListener("fetch", async (event) => {
    ...
    if (userMatch && method === "GET") {
      ...
      if (user) {
        ...
        event.respondWith(Response.json({ error: "User not found" }, { status: 404 }));
      }
      return;
    }
    ...
    if (pathname === "/api/users" && method === "POST") {
      try {
        ...
        event.respondWith(Response.json({ error: "Invalid JSON body" }, { status: 400 }));
      }
      return;
    }

    // 404 for all other routes
    event.respondWith(Response.json({ error: "Not Found", path: pathname }, { status: 404 }));
  });
  ...
  console.log(
    '  curl -X POST -H "Content-Type: application/json" -d \'{"name":"Dave","email":"dave@example.com"}\' http://localhost:3000/api/users',
  );
}
